...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/", "jenis-perjalanan-dinas", "Jenis Perjalanan Dinas");
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td>
							<input class="form-control" type="number" step="1" name="sort_order" id="sort_order" value="<?php echo $jenisPerjalananDinas->getSortOrder();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<button type="button" class="btn btn-primary" onclick="window.location='<?php echo $currentModule->getRedirectUrl();?>';"><?php echo $appLanguage->getButtonCancel();?></button>
							<input type="hidden" name="jenis_perjalanan_dinas_id" value="<?php echo $jenisPerjalananDinas->getJenisPerjalananDinasId();?>"/>
						</td>
					</tr>
				</tbody>
			</table>
...
<div class="page page-jambi page-detail">
	<div class="jambi-wrapper">
		<?php
		if(UserAction::isRequireNextAction($inputGet) && UserAction::isRequireApproval($jenisPerjalananDinas->getWaitingFor()))
		{
				?>
				<div class="alert alert-info"><?php echo UserAction::getWaitingForMessage($appLanguage, $jenisPerjalananDinas->getWaitingFor());?></div>
				<?php
		}
		?>
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td><?php echo $jenisPerjalananDinas->getSortOrder();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($jenisPerjalananDinas->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
					...
					<table class="table table-row table-sort-by-column">
						<thead>
							<tr>
								...
								<td class="data-controll data-selector" data-key="jenis_perjalanan_dinas_id">
									<input type="checkbox" class="checkbox check-master" data-selector=".checkbox-jenis-perjalanan-dinas-id"/>
								</td>
								<?php } ?>
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							<?php 
							$dataIndex = 0;
							while($jenisPerjalananDinas = $pageData->fetch())
							{
								$dataIndex++;
							?>
							...
							<tr data-primary-key="<?php echo $jenisPerjalananDinas->getJenisPerjalananDinasId();?>" data-sort-order="<?php echo $jenisPerjalananDinas->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>">
								...
								<?php } ?>
								<?php if($userPermission->isAllowedBatchAction()){ ?>
								<td class="data-selector" data-key="jenis_perjalanan_dinas_id">
									<input type="checkbox" class="checkbox check-slave checkbox-jenis-perjalanan-dinas-id" name="checked_row_id[]" value="<?php echo $jenisPerjalananDinas->getJenisPerjalananDinasId();?>"/>
								</td>
								<?php } ?>
								<?php if($userPermission->isAllowedUpdate()){ ?>
								<td>
									<a class="edit-control" href="<?php echo $currentModule->getRedirectUrl(UserAction::UPDATE, Field::of()->jenis_perjalanan_dinas_id, $jenisPerjalananDinas->getJenisPerjalananDinasId());?>"><span class="fa fa-edit"></span></a>
								</td>
								<?php } ?>
								<?php if($userPermission->isAllowedDetail()){ ?>
								<td>
									<a class="detail-control field-master" href="<?php echo $currentModule->getRedirectUrl(UserAction::DETAIL, Field::of()->jenis_perjalanan_dinas_id, $jenisPerjalananDinas->getJenisPerjalananDinasId());?>"><span class="fa fa-folder"></span></a>
								</td>
								<?php } ?>
