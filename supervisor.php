...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$supervisor->setSortOrder($inputPost->getSortOrder(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$supervisorApv->setSortOrder($inputPost->getSortOrder(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
...
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$supervisor->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->supervisorId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
	$currentModule->redirectToItself();
}
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getPassword();?></td>
						<td>
							<input class="form-control" type="password" name="password" id="password" value="<?php echo $supervisor->getPassword();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td>
							<input class="form-control" type="number" step="1" name="sort_order" id="sort_order" value="<?php echo $supervisor->getSortOrder();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIpTerakhirAktif();?></td>
			...
			<table class="responsive responsive-three-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td>
							<span class="<?php echo AppFormBuilder::classCompareData($supervisor->notEqualsSortOrder($supervisorApv->getSortOrder()));?>"><?php echo $supervisor->getSortOrder();?></span>
						</td>
						<td>
							<span class="<?php echo AppFormBuilder::classCompareData($supervisor->notEqualsSortOrder($supervisorApv->getSortOrder()));?>"><?php echo $supervisorApv->getSortOrder();?></span>
						</td>
					</tr>
					<tr>
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td><?php echo $supervisor->getSortOrder();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getWaktuBuat();?></td>
					...
					<table class="table table-row table-sort-by-column">
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $supervisor->getSupervisorId();?>" data-sort-order="<?php echo $supervisor->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>">
								...
								<td data-col-name="sort_order" class="data-sort-order-column"><?php echo $supervisor->getSortOrder();?></td>
