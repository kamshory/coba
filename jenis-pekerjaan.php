...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/", "jenis-pekerjaan", "Jenis Pekerjaan");
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$jenisPekerjaan->setKelasPondasiId($inputPost->getKelasPondasiId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$jenisPekerjaan->setTipeTowerId($inputPost->getTipeTowerId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$jenisPekerjaan->setLokasiProyekId($inputPost->getLokasiProyekId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$jenisPekerjaan->setKegiatan($inputPost->getKegiatan(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	...
	$jenisPekerjaan->setAdminBuat($currentUser->getUserId());
	$jenisPekerjaan->setWaktuBuat($currentAction->getTime());
	$jenisPekerjaan->setIpBuat($currentAction->getIp());
	$jenisPekerjaan->setAdminUbah($currentUser->getUserId());
...
else if($inputPost->getUserAction() == UserAction::DELETE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$jenisPekerjaan = new JenisPekerjaan(null, $database);
			$jenisPekerjaan->deleteOneByJenisPekerjaanId($rowId);
		}
	}
	$currentModule->redirectToItself();
}
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$jenisPekerjaan->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->jenisPekerjaanId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
	$currentModule->redirectToItself();
}
		...
		<form name="createform" id="createform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getKelasPondasiId();?></td>
						<td>
							<label><input class="form-check-input" type="checkbox" name="kelas_pondasi_id" id="kelas_pondasi_id" value="1"/> <?php echo $appEntityLanguage->getKelasPondasiId();?></label>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getTipeTowerId();?></td>
						<td>
							<label><input class="form-check-input" type="checkbox" name="tipe_tower_id" id="tipe_tower_id" value="1"/> <?php echo $appEntityLanguage->getTipeTowerId();?></label>
						</td>
					</tr>
					<tr>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getKelasPondasiId();?></td>
						<td>
							<label><input class="form-check-input" type="checkbox" name="kelas_pondasi_id" id="kelas_pondasi_id" value="1" <?php echo $jenisPekerjaan->createCheckedKelasPondasiId();?>/> <?php echo $appEntityLanguage->getKelasPondasiId();?></label>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getTipeTowerId();?></td>
						<td>
							<label><input class="form-check-input" type="checkbox" name="tipe_tower_id" id="tipe_tower_id" value="1" <?php echo $jenisPekerjaan->createCheckedTipeTowerId();?>/> <?php echo $appEntityLanguage->getTipeTowerId();?></label>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getLokasiProyekId();?></td>
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getKelasPondasiId();?></td>
						<td><?php echo $jenisPekerjaan->optionKelasPondasiId($appLanguage->getYes(), $appLanguage->getNo());?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getTipeTowerId();?></td>
						<td><?php echo $jenisPekerjaan->optionTipeTowerId($appLanguage->getYes(), $appLanguage->getNo());?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getLokasiProyekId();?></td>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($jenisPekerjaan->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							...
							<?php } else if($inputGet->getNextAction() == UserAction::REJECT && UserAction::isRequireApproval($jenisPekerjaan->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
