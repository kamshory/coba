...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$modul->setMenu($inputPost->getMenu(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$modul->setUrl($inputPost->getUrl(PicoFilterConstant::FILTER_SANITIZE_SPECIAL_CHARS, false, false, true));
	$modul->setIstimewa($inputPost->getIstimewa(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	...
	$modul->setAdminBuat($currentUser->getUserId());
	$modul->setWaktuBuat($currentAction->getTime());
	$modul->setIpBuat($currentAction->getIp());
	$modul->setAdminUbah($currentUser->getUserId());
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$modul->setWaktuUbah($currentAction->getTime());
	$modul->setIpUbah($currentAction->getIp());
	$modul->setModulId($inputPost->getModulId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$modul->update();
	$newId = $modul->getModulId();
		...
		foreach($inputPost->getCheckedRowId() as $rowId)
		{
			...
			try
			{
				$modul->where(PicoSpecification::getInstance()
					->addAnd(PicoPredicate::getInstance()->equals(Field::of()->modul_id, $rowId))
...
else if($inputPost->getUserAction() == UserAction::DELETE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$modul = new Modul(null, $database);
			$modul->deleteOneByModulId($rowId);
		}
	}
...
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$modul->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->modulId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						...
						<td>
							<input autocomplete="off" class="form-control" type="text" name="kode_modul" id="kode_modul"/>
						</td>
					</tr>
					<tr>
...
else if($inputGet->getUserAction() == UserAction::UPDATE)
{
	$modul = new Modul(null, $database);
	try{
		$modul->findOneByModulId($inputGet->getModulId());
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getKodeModul();?></td>
						<td>
							<input class="form-control" type="text" name="kode_modul" id="kode_modul" value="<?php echo $modul->getKodeModul();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
						<td>
							<input class="form-control" type="text" name="nama" id="nama" value="<?php echo $modul->getNama();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIcon();?></td>
						<td>
							<input class="form-control" type="text" name="icon" id="icon" value="<?php echo $modul->getIcon();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getGrupModul();?></td>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getUrl();?></td>
						<td>
							<input class="form-control" type="text" name="url" id="url" value="<?php echo $modul->getUrl();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIstimewa();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		...
		$modul->findOneWithPrimaryKeyValue($inputGet->getModulId(), $subqueryMap);
		if($modul->hasValueModulId())
		{
$appEntityLanguage = new AppEntityLanguage(new Modul(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						<td><?php echo $appEntityLanguage->getGrupModul();?></td>
						<td><?php echo $modul->hasValueGrupModul() ? $modul->getGrupModul()->getNama() : "";?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getMenu();?></td>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVAL && UserAction::isRequireApproval($modul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							...
							<?php } else if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($modul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-success" name="user_action" value="<?php echo UserAction::APPROVE;?>"><?php echo $appLanguage->getButtonApprove();?></button>
							<?php } else if($inputGet->getNextAction() == UserAction::REJECT && UserAction::isRequireApproval($modul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-warning" name="user_action" value="<?php echo UserAction::REJECT;?>"><?php echo $appLanguage->getButtonReject();?></button>
		...
		<div class="filter-section">
			<form action="" method="get" class="filter-form">
				...
				<span class="filter-group">
					...
					<span class="filter-control">
							<select name="grup_modul_id" class="form-control">
								<option value=""><?php echo $appLanguage->getLabelOptionSelectOne();?></option>
								<?php echo AppFormBuilder::getInstance()->createSelectOption(new GrupModul(null, $database), 
								PicoSpecification::getInstance()
									->addAnd(new PicoPredicate(Field::of()->aktif, true))
					...
					<table class="table table-row table-sort-by-column">
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $modul->getModulId();?>" data-sort-order="<?php echo $modul->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>">
								...
								<td data-col-name="grup_modul_id"><?php echo $modul->hasValueGrupModul() ? $modul->getGrupModul()->getNama() : "";?></td>
								...
								<td data-col-name="default_data"><?php echo $modul->optionDefaultData($appLanguage->getYes(), $appLanguage->getNo());?></td>
