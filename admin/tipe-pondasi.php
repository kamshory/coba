...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$tipePondasi->setTipePondasiId($inputPost->getTipePondasiId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	...
	$tipePondasi->setAdminBuat($currentUser->getUserId());
	$tipePondasi->setWaktuBuat($currentAction->getTime());
	$tipePondasi->setIpBuat($currentAction->getIp());
	$tipePondasi->setAdminUbah($currentUser->getUserId());
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$specification = PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->tipePondasiId, $inputPost->getTipePondasiId()));
	$tipePondasi = new TipePondasi(null, $database);
	$tipePondasi->where($specification)->setTipePondasiId($inputPost->getAppBuilderNewPkTipePondasiId())->update();
	$newId = $inputPost->getAppBuilderNewPkTipePondasiId();
	$currentModule->redirectTo(UserAction::DETAIL, Field::of()->tipe_pondasi_id, $newId);
}
...
else if($inputPost->getUserAction() == UserAction::DELETE)
{
	if($inputPost->countableCheckedRowId())
	{
		foreach($inputPost->getCheckedRowId() as $rowId)
		{
			...
			{
				$tipePondasi = new TipePondasi(null, $database);
				$tipePondasi->deleteOneByTipePondasiId($rowId);
			}
			catch(Exception $e)
			{
				// Do something here to handle exception
...
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$tipePondasi->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->tipePondasiId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
	$currentModule->redirectToItself();
}
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						...
						<td>
							<input autocomplete="off" class="form-control" type="number" step="1" name="tipe_pondasi_id" id="tipe_pondasi_id"/>
						</td>
					</tr>
					<tr>
...
else if($inputGet->getUserAction() == UserAction::UPDATE)
{
	$tipePondasi = new TipePondasi(null, $database);
	try{
		$tipePondasi->findOneByTipePondasiId($inputGet->getTipePondasiId());
		if($tipePondasi->hasValueTipePondasiId())
		{
$appEntityLanguage = new AppEntityLanguage(new TipePondasi(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
?>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getTipePondasiId();?></td>
						<td>
							<input class="form-control" type="number" step="1" name="app_builder_new_pk_tipe_pondasi_id" id="tipe_pondasi_id" value="<?php echo $tipePondasi->getTipePondasiId();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
						<td>
							<input class="form-control" type="text" name="nama" id="nama" value="<?php echo $tipePondasi->getNama();?>" autocomplete="off" required="required"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getSortOrder();?></td>
						<td>
							<input class="form-control" type="number" step="1" name="sort_order" id="sort_order" value="<?php echo $tipePondasi->getSortOrder();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		$subqueryMap = null;
		$tipePondasi->findOneWithPrimaryKeyValue($inputGet->getTipePondasiId(), $subqueryMap);
		if($tipePondasi->hasValueTipePondasiId())
		{
$appEntityLanguage = new AppEntityLanguage(new TipePondasi(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
		...
		<form name="detailform" id="detailform" action="" method="post">
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getTipePondasiId();?></td>
						<td><?php echo $tipePondasi->getTipePondasiId();?></td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVAL && UserAction::isRequireApproval($tipePondasi->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							...
							<?php } else if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($tipePondasi->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-success" name="user_action" value="<?php echo UserAction::APPROVE;?>"><?php echo $appLanguage->getButtonApprove();?></button>
							<?php } else if($inputGet->getNextAction() == UserAction::REJECT && UserAction::isRequireApproval($tipePondasi->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-warning" name="user_action" value="<?php echo UserAction::REJECT;?>"><?php echo $appLanguage->getButtonReject();?></button>
				...
				<div class="data-wrapper">
					<table class="table table-row table-sort-by-column">
						...
						<tbody class="data-table-manual-sort" data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-primary-key="<?php echo $tipePondasi->getTipePondasiId();?>" data-sort-order="<?php echo $tipePondasi->getSortOrder();?>" data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>">
								<?php if($userPermission->isAllowedSortOrder()){ ?>
								...
								<td data-col-name="tipe_pondasi_id"><?php echo $tipePondasi->getTipePondasiId();?></td>
