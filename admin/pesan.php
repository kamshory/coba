...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/admin", "pesan", "Pesan");
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$tautanPesan = $database->generateNewId();
	$pesan->setTautanPesan($tautanPesan);
	$pesan->setPengirimUserId($currentUser->getUserId());
	...
	$pesan->setIsi($inputPost->getIsi(PicoFilterConstant::FILTER_DEFAULT, false, false, true));
	$pesan->setDibaca(false);
	$pesan->setSalinan(false);
	$pesan->setWaktuBaca(null);
	$pesan->setAdminBuat($currentAction->getUserId());
	$pesan->setWaktuBuat($currentAction->getTime());
	$pesan->setIpBuat($currentAction->getIp());
	$pesan->setAdminUbah($currentAction->getUserId());
	$pesan->setWaktuUbah($currentAction->getTime());
	$pesan->setIpUbah($currentAction->getIp());

	$pesan->insert();

	$pesan->setSalinan(true);
	$pesan->unsetPesanId();
	$pesan->insert();
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$pesan->setWaktuUbah($currentAction->getTime());
	$pesan->setIpUbah($currentAction->getIp());
	$pesan->setPesanId($inputPost->getPesanId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$pesan->update();
	$newId = $pesan->getPesanId();
	$currentModule->redirectTo(UserAction::DETAIL, Field::of()->pesan_id, $newId);
}
				...
				$specification = PicoSpecification::getInstance()
					...
					->addAnd(PicoSpecification::getInstance()
						->addOr(PicoSpecification::getInstance()
							->addAnd(PicoPredicate::getInstance()->equals('pengirimUserId', $currentUser->getUserId()))
							->addAnd(PicoPredicate::getInstance()->equals('salinan', true))
						)
						->addOr(PicoSpecification::getInstance()
							->addAnd(PicoPredicate::getInstance()->equals('penerimaUserId', $currentUser->getUserId()))
				...
				summernoteAtMention: {
					getSuggestions: (value) => {
						...
						return userEmails.filter((email) => {
							return email.includes(value) && email !== value;
						});
					},
				},
			},
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					...
					<tr>
						...
						<td>
							<input autocomplete="off" class="form-control" type="text" name="subjek" id="subjek"/>
						</td>
					</tr>
					<tr>
...
<div class="page page-jambi page-detail">
	<div class="jambi-wrapper">
		<?php
		if(UserAction::isRequireNextAction($inputGet) && UserAction::isRequireApproval($pesan->getWaitingFor()))
		{
				?>
				<div class="alert alert-info"><?php echo UserAction::getWaitingForMessage($appLanguage, $pesan->getWaitingFor());?></div>
				<?php
		}
		?>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						...
						<td>
							<?php echo $pesan->issetPengirimUser() ? $pesan->getPengirimUser()->getFirstName() : "";?>
					...
					<tr>
						...
						<td>
							<?php echo $pesan->issetPenerimaUser() ? $pesan->getPenerimaUser()->getFirstName() : "";?>
					...
					<tr>
						...
						<td><?php echo $pesan->getWaktuBuat();?></td>
					</tr>
				</tbody>
			</table>
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
					...
					<table class="table table-row table-sort-by-column">
						...
						<tbody data-offset="<?php echo $pageData->getDataOffset();?>">
							...
							<tr data-number="<?php echo $pageData->getDataOffset() + $dataIndex;?>">
								...
								echo $messageUtil->getAudienceFromUser($pesan, $currentUser->getUserId());
								?></td>
								<td data-col-name="subjek"><?php echo $pesan->getSubjek();?></td>
								<td data-col-name="waktu_buat"><?php echo $pesan->getWaktuBuat();?></td>
							</tr>
							<?php 
							}
							?>
