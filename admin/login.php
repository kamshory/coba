...
if($inputPost->getPassword() != null && $inputPost->getUsername() != null)
{
    try
    {
        ...
        if($loginRule == null)
        {
            ...
            $loginRule->stInvalidLoginMaximumCount(5);
        }
        ...
        $specs = PicoSpecification::getInstance()
            ->addAnd(PicoPredicate::getInstance()->equals('grupPengguna', 'user'))
        ...
        $appIserImpl->findOneByUsernameAndActive($username, true);
        ...
        if(!$appIserImpl->equalsPassword($password2))
        {
            ...
            $catatanSalahLoginCreate
                ->setGrupPengguna('user')
                ->setUserId($appIserImpl->getUserId())
                ->setWaktuBuat($now)
                ->insert();
        }
        else if($appIserImpl->getBlocked())
        {
            // user blocked
            $sessions->adminUsername = $username;
