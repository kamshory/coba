...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/admin", "grup-modul", "Grup Modul");
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$grupModul->setAdminBuat($currentUser->getUserId());
	$grupModul->setWaktuBuat($currentAction->getTime());
	$grupModul->setIpBuat($currentAction->getIp());
	$grupModul->setAdminUbah($currentUser->getUserId());
...
else if($inputPost->getUserAction() == UserAction::UPDATE)
{
	...
	$grupModul->setWaktuUbah($currentAction->getTime());
	$grupModul->setIpUbah($currentAction->getIp());
	$grupModul->setGrupModulId($inputPost->getGrupModulId(PicoFilterConstant::FILTER_SANITIZE_NUMBER_INT, false, false, true));
	$grupModul->update();
	$newId = $grupModul->getGrupModulId();
		...
		foreach($inputPost->getCheckedRowId() as $rowId)
		{
			...
			try
			{
				$grupModul->where(PicoSpecification::getInstance()
					->addAnd(PicoPredicate::getInstance()->equals(Field::of()->grup_modul_id, $rowId))
...
else if($inputPost->getUserAction() == UserAction::DELETE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$grupModul = new GrupModul(null, $database);
			$grupModul->deleteOneByGrupModulId($rowId);
		}
	}
...
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$grupModul->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->grupModulId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
...
else if($inputGet->getUserAction() == UserAction::UPDATE)
{
	$grupModul = new GrupModul(null, $database);
	try{
		$grupModul->findOneByGrupModulId($inputGet->getGrupModulId());
		if($grupModul->hasValueGrupModulId())
		{
$appEntityLanguage = new AppEntityLanguage(new GrupModul(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
?>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td><?php echo $appEntityLanguage->getNama();?></td>
						<td>
							<input class="form-control" type="text" name="nama" id="nama" value="<?php echo $grupModul->getNama();?>" autocomplete="off" required="required"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getIcon();?></td>
						<td>
							<input class="form-control" type="text" name="icon" id="icon" value="<?php echo $grupModul->getIcon();?>" autocomplete="off"/>
						</td>
					</tr>
					<tr>
						<td><?php echo $appEntityLanguage->getDefaultData();?></td>
...
else if($inputGet->getUserAction() == UserAction::DETAIL)
{
	...
	try{
		$subqueryMap = null;
		$grupModul->findOneWithPrimaryKeyValue($inputGet->getGrupModulId(), $subqueryMap);
		if($grupModul->hasValueGrupModulId())
		{
$appEntityLanguage = new AppEntityLanguage(new GrupModul(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVAL && UserAction::isRequireApproval($grupModul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							...
							<?php } else if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($grupModul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-success" name="user_action" value="<?php echo UserAction::APPROVE;?>"><?php echo $appLanguage->getButtonApprove();?></button>
							<?php } else if($inputGet->getNextAction() == UserAction::REJECT && UserAction::isRequireApproval($grupModul->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							<button type="submit" class="btn btn-warning" name="user_action" value="<?php echo UserAction::REJECT;?>"><?php echo $appLanguage->getButtonReject();?></button>
...
if($inputGet->getUserAction() == UserAction::EXPORT)
{
	...
	$exporter->write($pageData, $fileName, $sheetName, array(
		...
		$appEntityLanguage->getIcon() => $headerFormat->getIcon(),
