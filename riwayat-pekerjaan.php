...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/", "riwayat-pekerjaan", "Riwayat Pekerjaan");
...
else if($inputPost->getUserAction() == UserAction::ACTIVATE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$pekerjaan = new Pekerjaan(null, $database);
			$pekerjaan->setPekerjaanId($rowId)->setAktif(true)->update();
		}
	}
...
else if($inputPost->getUserAction() == UserAction::DEACTIVATE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$pekerjaan = new Pekerjaan(null, $database);
			$pekerjaan->setPekerjaanId($rowId)->setAktif(false)->update();
		}
	}
	$currentModule->redirectToItself();
}
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($userPermission->isAllowedUpdate()){ ?><button type="button" class="btn btn-primary" onclick="window.location='<?php echo $currentModule->getRedirectUrl(UserAction::UPDATE, Field::of()->pekerjaan_id, $pekerjaan->getPekerjaanId());?>';"><?php echo $appLanguage->getButtonUpdate();?></button><?php } ?>&#xD;
