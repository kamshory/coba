...
$currentModule = new PicoModule($appConfig, $database, $appModule, "/", "acuan-pengawasan", "Acuan Pengawasan");
...
if($inputPost->getUserAction() == UserAction::CREATE)
{
	...
	$acuanPengawasan->setAdminBuat($currentUser->getUserId());
	$acuanPengawasan->setWaktuBuat($currentAction->getTime());
	$acuanPengawasan->setIpBuat($currentAction->getIp());
	$acuanPengawasan->setAdminUbah($currentUser->getUserId());
		...
		foreach($inputPost->getCheckedRowId() as $rowId)
		{
			...
			try
			{
				$acuanPengawasan->where(PicoSpecification::getInstance()
					->addAnd(PicoPredicate::getInstance()->equals(Field::of()->acuan_pengawasan_id, $rowId))
...
else if($inputPost->getUserAction() == UserAction::DELETE)
{
	if($inputPost->countableCheckedRowId())
	{
		...
		{
			$acuanPengawasan = new AcuanPengawasan(null, $database);
			$acuanPengawasan->deleteOneByAcuanPengawasanId($rowId);
		}
	}
	$currentModule->redirectToItself();
}
else if($inputPost->getUserAction() == UserAction::SORT_ORDER)
{
	...
	if($inputPost->getNewOrder() != null && $inputPost->countableNewOrder())
	{
		foreach($inputPost->getNewOrder() as $dataItem)
		{
			...
			$acuanPengawasan->where(PicoSpecification::getInstance()->addAnd(new PicoPredicate(Field::of()->acuanPengawasanId, $primaryKeyValue)))->setSortOrder($sortOrder)->update();
		}
	}
	$currentModule->redirectToItself();
}
...
else if($inputGet->getUserAction() == UserAction::UPDATE)
{
	$acuanPengawasan = new AcuanPengawasan(null, $database);
	try{
		$acuanPengawasan->findOneByAcuanPengawasanId($inputGet->getAcuanPengawasanId());
		if($acuanPengawasan->hasValueAcuanPengawasanId())
		{
$appEntityLanguage = new AppEntityLanguage(new AcuanPengawasan(), $appConfig, $currentUser->getLanguageId());
require_once $appInclude->mainAppHeader(__DIR__);
?>
			...
			<table class="responsive responsive-two-cols" border="0" cellpadding="0" cellspacing="0" width="100%">
				<tbody>
					<tr>
						<td></td>
						<td>
							<?php if($inputGet->getNextAction() == UserAction::APPROVE && UserAction::isRequireApproval($acuanPengawasan->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
							...
							<?php } else if($inputGet->getNextAction() == UserAction::REJECT && UserAction::isRequireApproval($acuanPengawasan->getWaitingFor()) && $userPermission->isAllowedApprove()){ ?>
